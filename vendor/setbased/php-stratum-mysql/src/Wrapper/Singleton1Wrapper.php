...
class Singleton1Wrapper extends MysqlWrapper
{
  ...
  protected function generateMethodBodyWithLobReturnData(WrapperContext $context): void
  {
    ...
    $context->codeStore->append('{');
    $context->codeStore->append('throw new ResultException([1], sizeof($tmp), $query);');
    $context->codeStore->append('}');
    $context->codeStore->append('');
    ...
    {
      $context->codeStore->append('return $tmp[0][0];');
    }
  }
  ...
  protected function generateMethodBodyWithoutLob(WrapperContext $context): void
  {
    ...
    if ($context->phpStratumMetadata['designation']['return']===['bool'])
    {
      $context->codeStore->append(sprintf("return !empty(\$this->executeSingleton1('call %s(%s)'));",
                                          $context->phpStratumMetadata['routine_name'],
                                          $this->getRoutineArgs($context)));
    }
    else
    {
      $context->codeStore->append(sprintf("return \$this->executeSingleton1('call %s(%s)');",
                                          $context->phpStratumMetadata['routine_name'],
                                          $this->getRoutineArgs($context)));
    }
  }
